...
> 语义信号健康由 KB 引擎在写入时守护：写回前可用 **灯笼 KB 连接器（lantern-kb）** 的 `kb_state`（见仓库 AGENTS.md）查看独立性 r 与信号状态；embedding 退化时引擎自动挂起语义链，无需 Skill 侧额外命令。
...
- `projection` 必须含至少一个**因果判断或对比关系**，**禁止**纯描述（如"通货膨胀是指物价上涨"）；长度建议 ≥20 字（引擎对过短投影扣分，见 `update-scores` 的计分逻辑）。
