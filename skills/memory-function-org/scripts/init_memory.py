...
> 每张索引表每行须带「一句话摘要 / 关键问题」列（类比 Skill 的 description 字段）：
> 用一句讲清该文档意义或所解决问题，使检索时扫摘要即可判定要不要点开，不必读文件。

## 分区一 · 经验沉淀（具体环节精髓 → functions/）

| 功能模块 | 一句话摘要 / 关键问题 | 迭代记录 | 状态 |
|---|---|---|---|
| （示例）某功能 | 一句话讲清它做什么 / 踩过什么坑 | [functions/某功能.md](functions/某功能.md) | 待补充 |

## 分区二 · 创新优化思想（可通用·思想迭代升华 → insights/）

| 思想主题 | 一句话摘要 / 解决什么取舍 | 思想文件 | 源于（蒸馏自） |
|---|---|---|---|
| （示例）某思想 | 一句话讲清这条原则 / 它解决什么取舍 | [insights/某思想.md](insights/某思想.md) | [functions/某功能.md](functions/某功能.md) |

## 功能详档索引（详细论证下沉至 functions/，本文件只留指针）
- （示例）某功能的完整推导 / API / 领域谱系 → [functions/某功能.md](functions/某功能.md)
- 注意：MEMORY.md 只承载「索引 + 跨功能铁律/共识 + 环境常量」；某功能的具体详档写在对应 functions 文件里，这里仅放指针。
